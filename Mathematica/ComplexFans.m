...
INDivision[IN[FE[fe1_],SE[se1_],FEI[fei1_],SEI[sei1_]],IN[FE[fe2_],SE[se2_],FEI[fei2_],SEI[sei2_]]]:=
	If[se2==0||fe2==0,
		Print["Division by zero! ","res=in1/in2;", " in1=",INStringForm[IN[FE[fe1],SE[se1],FEI[fei1],SEI[sei1]]],"; in2=",INStringForm[IN[FE[fe2],SE[se2],FEI[fei2],SEI[sei2]]],";"];
		IN[FE[N[fe1/se2]],SE[N[se1/fe2]],FEI[If[fei1=="["&&sei2=="]",fei1,"("]],SEI[If[sei1=="]"&&fei2=="[",sei1,")"]]],
		IN[FE[N[fe1/se2]],SE[N[se1/fe2]],FEI[If[fei1=="["&&sei2=="]",fei1,"("]],SEI[If[sei1=="]"&&fei2=="[",sei1,")"]]]
	];
...
CFAddition[cf1_,cf2_]:=
	Block[{inters,ptotal,paux,acf1,acf2,total,res,i,j,k,rot1,rot2,caso,aux,auxai,mi1,mi2,ai1,ai2,V1,V2,pcf1,pcf2,rotation},
		...
		acf1=cf1;
		acf2=cf2;
		(* ensuring that alfa1 will be zero *)
		...
		For[i=1,i<=Length[pcf1],i++,
			For[j=1,j<=Length[pcf2],j++,
				...
				Which[
					...
					True,
					(* V1 is in the first quadrant and V2 in the fourth *)
					ai1=CFGetAI[V1];
					ai2=CFGetAI[V2];
					rot2 = INGetFE[ai2];
...
CFAngleCase2[cf1_,cf2_]:=
	Block[{aux1,aux2,aux3,aux4,interseccion,V1a,V2a,a,b,c,d,alfa1,alfa2,alfa3,alfa4,alfa5,alfa6,anguloy,anguloo,mi1=CFGetMI[cf1],mi2=CFGetMI[cf2],ai1=CFGetAI[cf1],ai2=CFGetAI[cf2]},
		...
		If[alfa5>90,
			...
			If[!INEmptyQ[interseccion],
				(* Correction of the algorithm from alfa5=anguloo to alfa5=anguloy*)
				alfa5 = anguloy;,
				If[anguloo<alfa1,
					(* alfa5 = p25a *)
...
(* It plots a list of complex fans *)
Clear[CFPlot];
CFPlot[lcf_]:=
	Block[{},
		If[ListQ[lcf],
			If[Length[lcf]>0,
				Graphics[Map[CFBeforePlot,lcf],Axes->True,AxesOrigin->{0,0},AxesStyle->Gray,Method->{"AxesInFront"->False}]
			]
		]
	];
...
CFAdditionPlot[cf1_,cf2_,cfr_]:=
	Block[{cf1g,cf2g,cfrg,aux1,aux2,aux3,aux4,aux5,aux6,aux7,aux8,mi1=CFGetMI[cf1],ai1=CFGetAI[cf1],mi2=CFGetMI[cf2],ai2=CFGetAI[cf2],a,b,alfa1,alfa2,c,d,alfa3,alfa4},
		...
		{Graphics[Join[{Dashing[Tiny],Green},aux1,aux2,aux3,aux4,{Red,Dashing[{}]},cfrg,{Blue},cf1g,{Green},cf2g],Axes->True,AxesStyle->Gray,AxesOrigin->{0,0},Method->{"AxesInFront"->False}],Graphics[Join[{Dashing[Tiny],Blue},aux5,aux6,aux7,aux8,{Red,Dashing[{}]},cfrg,{Blue},cf1g,{Green},cf2g],Axes->True,AxesStyle->Gray,AxesOrigin->{0,0},Method->{"AxesInFront"->False}]}
...
CFAdditionPlotAux[p_,cf_]:=
	Block[{mi=CFGetMI[cf],ai=CFGetAI[cf],a,b,alfa1,alfa2},
		...
		{Circle[p,a,{alfa1 Degree,alfa2 Degree}],Circle[p,b,{alfa1 Degree,alfa2 Degree}],Line[{p,{b Cos[alfa1 Degree]+p[[1]],b Sin[alfa1 Degree]+p[[2]]}}],Line[{p,{b Cos[alfa2 Degree]+p[[1]],b Sin[alfa2 Degree]+p[[2]]}}]}
	];
